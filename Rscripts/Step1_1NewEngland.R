...
dir_grid_urban <- '/media/qnap3/Exposure modeling/3 National scale/USA/7 PM2.5 speciation v1.2000_19/1 New England/Urban areas at 50m spatial resolution/'
dir_grid_nonurban <- '/media/qnap3/Exposure modeling/3 National scale/USA/7 PM2.5 speciation v1.2000_19/1 New England/Non-urban areas at 1km spatial resolution/'
...
############################# 1. br ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'br_20',i,'_NE.rds'))){
    ### combine urban and nonurban grids
    br_grid_urban <- readRDS(paste0(dir_grid_urban,'20',i,'.final.predictions.urban.NE1.50m.br.rds'))
    br_grid_urban <- br_grid_urban[,c("lon","lat","final.predicted.br")]
    br_grid_nonurban <- readRDS(paste0(dir_grid_nonurban,'final.predictions.nonurban.NE1.br.rds'))
    ...
    saveRDS(br_combine,file = paste0(dir_save,'br_20',i,'_NE.rds'))
    
    rm(br_grid_urban,br_grid_nonurban,br_grid,br_grid_zipcode,cty,br_zip,br_grid_pobox,pobox,link,br_combine)
    gc()
  }
  cat(i,'\n')
}
...
############################# 2. ca ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'ca_20',i,'_NE.rds'))){
    ### combine urban and nonurban grids
    ca_grid_urban <- readRDS(paste0(dir_grid_urban,'20',i,'.final.predictions.urban.NE1.50m.ca.rds'))
    ca_grid_urban <- ca_grid_urban[,c("lon","lat","final.predicted.ca")]
    ca_grid_nonurban <- readRDS(paste0(dir_grid_nonurban,'final.predictions.nonurban.NE1.ca.rds'))
    ...
    saveRDS(ca_combine,file = paste0(dir_save,'ca_20',i,'_NE.rds'))
    
    rm(ca_grid_urban,ca_grid_nonurban,ca_grid,ca_grid_zipcode,cty,ca_zip,ca_grid_pobox,pobox,link,ca_combine)
    gc()
  }
  cat(i,'\n')
}
...
############################# 3. cu ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'cu_20',i,'_NE.rds'))){
    ### combine urban and nonurban grids
    cu_grid_urban <- readRDS(paste0(dir_grid_urban,'20',i,'.final.predictions.urban.NE1.50m.cu.rds'))
    cu_grid_urban <- cu_grid_urban[,c("lon","lat","final.predicted.cu")]
    cu_grid_nonurban <- readRDS(paste0(dir_grid_nonurban,'final.predictions.nonurban.NE1.cu.rds'))
    ...
    saveRDS(cu_combine,file = paste0(dir_save,'cu_20',i,'_NE.rds'))
    
    rm(cu_grid_urban,cu_grid_nonurban,cu_grid,cu_grid_zipcode,cty,cu_zip,cu_grid_pobox,pobox,link,cu_combine)
    gc()
  }
  cat(i,'\n')
}
...
############################# 4. ec ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'ec_20',i,'_NE.rds'))){
    ### combine urban and nonurban grids
    ec_grid_urban <- readRDS(paste0(dir_grid_urban,'20',i,'.final.predictions.urban.NE1.50m.ec.rds'))
    ec_grid_urban <- ec_grid_urban[,c("lon","lat","final.predicted.ec")]
    ec_grid_nonurban <- readRDS(paste0(dir_grid_nonurban,'final.predictions.nonurban.NE1.ec.rds'))
    ...
    saveRDS(ec_combine,file = paste0(dir_save,'ec_20',i,'_NE.rds'))
    
    rm(ec_grid_urban,ec_grid_nonurban,ec_grid,ec_grid_zipcode,cty,ec_zip,ec_grid_pobox,pobox,link,ec_combine)
    gc()
  }
  cat(i,'\n')
}
...
############################# 5. fe ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'fe_20',i,'_NE.rds'))){
    ### combine urban and nonurban grids
    fe_grid_urban <- readRDS(paste0(dir_grid_urban,'20',i,'.final.predictions.urban.NE1.50m.fe.rds'))
    fe_grid_urban <- fe_grid_urban[,c("lon","lat","final.predicted.fe")]
    fe_grid_nonurban <- readRDS(paste0(dir_grid_nonurban,'final.predictions.nonurban.NE1.fe.rds'))
    ...
    saveRDS(fe_combine,file = paste0(dir_save,'fe_20',i,'_NE.rds'))
    
    rm(fe_grid_urban,fe_grid_nonurban,fe_grid,fe_grid_zipcode,cty,fe_zip,fe_grid_pobox,pobox,link,fe_combine)
    gc()
  }
  cat(i,'\n')
}
...
############################# 6. k ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'k_20',i,'_NE.rds'))){
    ### combine urban and nonurban grids
    k_grid_urban <- readRDS(paste0(dir_grid_urban,'20',i,'.final.predictions.urban.NE1.50m.k.rds'))
    k_grid_urban <- k_grid_urban[,c("lon","lat","final.predicted.k")]
    k_grid_nonurban <- readRDS(paste0(dir_grid_nonurban,'final.predictions.nonurban.NE1.k.rds'))
    ...
    saveRDS(k_combine,file = paste0(dir_save,'k_20',i,'_NE.rds'))
    
    rm(k_grid_urban,k_grid_nonurban,k_grid,k_grid_zipcode,cty,k_zip,k_grid_pobox,pobox,link,k_combine)
    gc()
  }
  cat(i,'\n')
}
...
############################# 7. nh4 ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'nh4_20',i,'_NE.rds'))){
    ### combine urban and nonurban grids
    nh4_grid_urban <- readRDS(paste0(dir_grid_urban,'20',i,'.final.predictions.urban.NE1.50m.nh4.rds'))
    nh4_grid_urban <- nh4_grid_urban[,c("lon","lat","final.predicted.nh4")]
    nh4_grid_nonurban <- readRDS(paste0(dir_grid_nonurban,'final.predictions.nonurban.NE1.nh4.rds'))
    ...
    saveRDS(nh4_combine,file = paste0(dir_save,'nh4_20',i,'_NE.rds'))
    
    rm(nh4_grid_urban,nh4_grid_nonurban,nh4_grid,nh4_grid_zipcode,cty,nh4_zip,nh4_grid_pobox,pobox,link,nh4_combine)
    gc()
  }
  cat(i,'\n')
}
...
############################# 8. ni ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'ni_20',i,'_NE.rds'))){
    ### combine urban and nonurban grids
    ni_grid_urban <- readRDS(paste0(dir_grid_urban,'20',i,'.final.predictions.urban.NE1.50m.ni.rds'))
    ni_grid_urban <- ni_grid_urban[,c("lon","lat","final.predicted.ni")]
    ni_grid_nonurban <- readRDS(paste0(dir_grid_nonurban,'final.predictions.nonurban.NE1.ni.rds'))
    ...
    saveRDS(ni_combine,file = paste0(dir_save,'ni_20',i,'_NE.rds'))
    
    rm(ni_grid_urban,ni_grid_nonurban,ni_grid,ni_grid_zipcode,cty,ni_zip,ni_grid_pobox,pobox,link,ni_combine)
    gc()
  }
  cat(i,'\n')
}
...
############################# 9. no3 ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'no3_20',i,'_NE.rds'))){
    ### combine urban and nonurban grids
    no3_grid_urban <- readRDS(paste0(dir_grid_urban,'20',i,'.final.predictions.urban.NE1.50m.no3.rds'))
    no3_grid_urban <- no3_grid_urban[,c("lon","lat","final.predicted.no3")]
    no3_grid_nonurban <- readRDS(paste0(dir_grid_nonurban,'final.predictions.nonurban.NE1.no3.rds'))
    ...
    saveRDS(no3_combine,file = paste0(dir_save,'no3_20',i,'_NE.rds'))
    
    rm(no3_grid_urban,no3_grid_nonurban,no3_grid,no3_grid_zipcode,cty,no3_zip,no3_grid_pobox,pobox,link,no3_combine)
    gc()
  }
  cat(i,'\n')
}
...
############################# 10. oc ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'oc_20',i,'_NE.rds'))){
    ### combine urban and nonurban grids
    oc_grid_urban <- readRDS(paste0(dir_grid_urban,'20',i,'.final.predictions.urban.NE1.50m.oc.rds'))
    oc_grid_urban <- oc_grid_urban[,c("lon","lat","final.predicted.oc")]
    oc_grid_nonurban <- readRDS(paste0(dir_grid_nonurban,'final.predictions.nonurban.NE1.oc.rds'))
    ...
    saveRDS(oc_combine,file = paste0(dir_save,'oc_20',i,'_NE.rds'))
    
    rm(oc_grid_urban,oc_grid_nonurban,oc_grid,oc_grid_zipcode,cty,oc_zip,oc_grid_pobox,pobox,link,oc_combine)
    gc()
  }
  cat(i,'\n')
}
...
############################# 11. pb ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'pb_20',i,'_NE.rds'))){
    ### combine urban and nonurban grids
    pb_grid_urban <- readRDS(paste0(dir_grid_urban,'20',i,'.final.predictions.urban.NE1.50m.pb.rds'))
    pb_grid_urban <- pb_grid_urban[,c("lon","lat","final.predicted.pb")]
    pb_grid_nonurban <- readRDS(paste0(dir_grid_nonurban,'final.predictions.nonurban.NE1.pb.rds'))
    ...
    saveRDS(pb_combine,file = paste0(dir_save,'pb_20',i,'_NE.rds'))
    
    rm(pb_grid_urban,pb_grid_nonurban,pb_grid,pb_grid_zipcode,cty,pb_zip,pb_grid_pobox,pobox,link,pb_combine)
    gc()
  }
  cat(i,'\n')
}
...
############################# 12. si ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'si_20',i,'_NE.rds'))){
    ### combine urban and nonurban grids
    si_grid_urban <- readRDS(paste0(dir_grid_urban,'20',i,'.final.predictions.urban.NE1.50m.si.rds'))
    si_grid_urban <- si_grid_urban[,c("lon","lat","final.predicted.si")]
    si_grid_nonurban <- readRDS(paste0(dir_grid_nonurban,'final.predictions.nonurban.NE1.si.rds'))
    ...
    saveRDS(si_combine,file = paste0(dir_save,'si_20',i,'_NE.rds'))
    
    rm(si_grid_urban,si_grid_nonurban,si_grid,si_grid_zipcode,cty,si_zip,si_grid_pobox,pobox,link,si_combine)
    gc()
  }
  cat(i,'\n')
}
...
############################# 13. so4 ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'so4_20',i,'_NE.rds'))){
    ### combine urban and nonurban grids
    so4_grid_urban <- readRDS(paste0(dir_grid_urban,'20',i,'.final.predictions.urban.NE1.50m.so4.rds'))
    so4_grid_urban <- so4_grid_urban[,c("lon","lat","final.predicted.so4")]
    so4_grid_nonurban <- readRDS(paste0(dir_grid_nonurban,'final.predictions.nonurban.NE1.so4.rds'))
    ...
    saveRDS(so4_combine,file = paste0(dir_save,'so4_20',i,'_NE.rds'))
    
    rm(so4_grid_urban,so4_grid_nonurban,so4_grid,so4_grid_zipcode,cty,so4_zip,so4_grid_pobox,pobox,link,so4_combine)
    gc()
  }
  cat(i,'\n')
}
...
############################# 14. v ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'v_20',i,'_NE.rds'))){
    ### combine urban and nonurban grids
    v_grid_urban <- readRDS(paste0(dir_grid_urban,'20',i,'.final.predictions.urban.NE1.50m.v.rds'))
    v_grid_urban <- v_grid_urban[,c("lon","lat","final.predicted.v")]
    v_grid_nonurban <- readRDS(paste0(dir_grid_nonurban,'final.predictions.nonurban.NE1.v.rds'))
    ...
    saveRDS(v_combine,file = paste0(dir_save,'v_20',i,'_NE.rds'))
    
    rm(v_grid_urban,v_grid_nonurban,v_grid,v_grid_zipcode,cty,v_zip,v_grid_pobox,pobox,link,v_combine)
    gc()
  }
  cat(i,'\n')
}
...
############################# 15. z ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'z_20',i,'_NE.rds'))){
    ### combine urban and nonurban grids
    z_grid_urban <- readRDS(paste0(dir_grid_urban,'20',i,'.final.predictions.urban.NE1.50m.z.rds'))
    z_grid_urban <- z_grid_urban[,c("lon","lat","final.predicted.z")]
    z_grid_nonurban <- readRDS(paste0(dir_grid_nonurban,'final.predictions.nonurban.NE1.z.rds'))
    ...
    saveRDS(z_combine,file = paste0(dir_save,'z_20',i,'_NE.rds'))
    
    rm(z_grid_urban,z_grid_nonurban,z_grid,z_grid_zipcode,cty,z_zip,z_grid_pobox,pobox,link,z_combine)
    gc()
  }
  cat(i,'\n')
}
