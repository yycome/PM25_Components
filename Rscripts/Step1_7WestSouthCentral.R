...
dir_grid_urban <- '/media/qnap4/Exposure modeling predictions for PM2.5 composition -- continuation from QNAP3 EM directory/7 West South Central/Urban areas at 50m spatial resolution/'
dir_grid_nonurban <- '/media/qnap3/Exposure modeling/3 National scale/USA/7 PM2.5 speciation v1.2000_19/7 West South Central/Non-urban areas at 1km spatial resolution/'
...
############################# 1. br ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'br_20',i,'_WSC.rds'))){
    ### combine urban and nonurban grids
    br_grid_urban_files <- list.files(path=dir_grid_urban,pattern = paste0("^20",i,"(.*)final.predictions.urban.WSC7.50m.br.rds$"))
    ...
    br_grid_nonurban_files <- list.files(path=dir_grid_nonurban,pattern = paste0("^final.predictions.nonurban.WSC7(.*)br.rds$"))
    br_grid_nonurban <- readRDS(paste0(dir_grid_nonurban,br_grid_nonurban_files[1]))
    for (k in 2:length(br_grid_nonurban_files)) {
      br_grid_nonurban_tmp <- readRDS(paste0(dir_grid_nonurban,br_grid_nonurban_files[k]))
      br_grid_nonurban <- bind_rows(br_grid_nonurban,br_grid_nonurban_tmp)
      rm(br_grid_nonurban_tmp)
      gc()
    }
    ...
    saveRDS(br_combine,file = paste0(dir_save,'br_20',i,'_WSC.rds'))
...
############################# 2. ca ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'ca_20',i,'_WSC.rds'))){
    ### combine urban and nonurban grids
    ca_grid_urban_files <- list.files(path=dir_grid_urban,pattern = paste0("^20",i,"(.*)final.predictions.urban.WSC7.50m.ca.rds$"))
    ...
    ca_grid_nonurban_files <- list.files(path=dir_grid_nonurban,pattern = paste0("^final.predictions.nonurban.WSC7(.*)ca.rds$"))
    ca_grid_nonurban <- readRDS(paste0(dir_grid_nonurban,ca_grid_nonurban_files[1]))
    for (k in 2:length(ca_grid_nonurban_files)) {
      ca_grid_nonurban_tmp <- readRDS(paste0(dir_grid_nonurban,ca_grid_nonurban_files[k]))
      ca_grid_nonurban <- bind_rows(ca_grid_nonurban,ca_grid_nonurban_tmp)
      rm(ca_grid_nonurban_tmp)
      gc()
    }
    ...
    saveRDS(ca_combine,file = paste0(dir_save,'ca_20',i,'_WSC.rds'))
...
############################# 3. cu ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'cu_20',i,'_WSC.rds'))){
    ### combine urban and nonurban grids
    cu_grid_urban_files <- list.files(path=dir_grid_urban,pattern = paste0("^20",i,"(.*)final.predictions.urban.WSC7.50m.cu.rds$"))
    ...
    cu_grid_nonurban_files <- list.files(path=dir_grid_nonurban,pattern = paste0("^final.predictions.nonurban.WSC7(.*)cu.rds$"))
    cu_grid_nonurban <- readRDS(paste0(dir_grid_nonurban,cu_grid_nonurban_files[1]))
    for (k in 2:length(cu_grid_nonurban_files)) {
      cu_grid_nonurban_tmp <- readRDS(paste0(dir_grid_nonurban,cu_grid_nonurban_files[k]))
      cu_grid_nonurban <- bind_rows(cu_grid_nonurban,cu_grid_nonurban_tmp)
      rm(cu_grid_nonurban_tmp)
      gc()
    }
    ...
    saveRDS(cu_combine,file = paste0(dir_save,'cu_20',i,'_WSC.rds'))
...
############################# 4. ec ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'ec_20',i,'_WSC.rds'))){
    ### combine urban and nonurban grids
    ec_grid_urban_files <- list.files(path=dir_grid_urban,pattern = paste0("^20",i,"(.*)final.predictions.urban.WSC7.50m.ec.rds$"))
    ...
    ec_grid_nonurban_files <- list.files(path=dir_grid_nonurban,pattern = paste0("^final.predictions.nonurban.WSC7(.*)ec.rds$"))
    ec_grid_nonurban <- readRDS(paste0(dir_grid_nonurban,ec_grid_nonurban_files[1]))
    for (k in 2:length(ec_grid_nonurban_files)) {
      ec_grid_nonurban_tmp <- readRDS(paste0(dir_grid_nonurban,ec_grid_nonurban_files[k]))
      ec_grid_nonurban <- bind_rows(ec_grid_nonurban,ec_grid_nonurban_tmp)
      rm(ec_grid_nonurban_tmp)
      gc()
    }
    ...
    saveRDS(ec_combine,file = paste0(dir_save,'ec_20',i,'_WSC.rds'))
...
############################# 5. fe ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'fe_20',i,'_WSC.rds'))){
    ### combine urban and nonurban grids
    fe_grid_urban_files <- list.files(path=dir_grid_urban,pattern = paste0("^20",i,"(.*)final.predictions.urban.WSC7.50m.fe.rds$"))
    ...
    fe_grid_nonurban_files <- list.files(path=dir_grid_nonurban,pattern = paste0("^final.predictions.nonurban.WSC7(.*)fe.rds$"))
    fe_grid_nonurban <- readRDS(paste0(dir_grid_nonurban,fe_grid_nonurban_files[1]))
    for (k in 2:length(fe_grid_nonurban_files)) {
      fe_grid_nonurban_tmp <- readRDS(paste0(dir_grid_nonurban,fe_grid_nonurban_files[k]))
      fe_grid_nonurban <- bind_rows(fe_grid_nonurban,fe_grid_nonurban_tmp)
      rm(fe_grid_nonurban_tmp)
      gc()
    }
    ...
    saveRDS(fe_combine,file = paste0(dir_save,'fe_20',i,'_WSC.rds'))
...
############################# 6. k ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'k_20',i,'_WSC.rds'))){
    ### combine urban and nonurban grids
    k_grid_urban_files <- list.files(path=dir_grid_urban,pattern = paste0("^20",i,"(.*)final.predictions.urban.WSC7.50m.k.rds$"))
    ...
    k_grid_nonurban_files <- list.files(path=dir_grid_nonurban,pattern = paste0("^final.predictions.nonurban.WSC7(.*)k.rds$"))
    k_grid_nonurban <- readRDS(paste0(dir_grid_nonurban,k_grid_nonurban_files[1]))
    for (k in 2:length(k_grid_nonurban_files)) {
      k_grid_nonurban_tmp <- readRDS(paste0(dir_grid_nonurban,k_grid_nonurban_files[k]))
      k_grid_nonurban <- bind_rows(k_grid_nonurban,k_grid_nonurban_tmp)
      rm(k_grid_nonurban_tmp)
      gc()
    }
    ...
    saveRDS(k_combine,file = paste0(dir_save,'k_20',i,'_WSC.rds'))
...
############################# 7. nh4 ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'nh4_20',i,'_WSC.rds'))){
    ### combine urban and nonurban grids
    nh4_grid_urban_files <- list.files(path=dir_grid_urban,pattern = paste0("^20",i,"(.*)final.predictions.urban.WSC7.50m.nh4.rds$"))
    ...
    nh4_grid_nonurban_files <- list.files(path=dir_grid_nonurban,pattern = paste0("^final.predictions.nonurban.WSC7(.*)nh4.rds$"))
    nh4_grid_nonurban <- readRDS(paste0(dir_grid_nonurban,nh4_grid_nonurban_files[1]))
    for (k in 2:length(nh4_grid_nonurban_files)) {
      nh4_grid_nonurban_tmp <- readRDS(paste0(dir_grid_nonurban,nh4_grid_nonurban_files[k]))
      nh4_grid_nonurban <- bind_rows(nh4_grid_nonurban,nh4_grid_nonurban_tmp)
      rm(nh4_grid_nonurban_tmp)
      gc()
    }
    ...
    saveRDS(nh4_combine,file = paste0(dir_save,'nh4_20',i,'_WSC.rds'))
...
############################# 8. ni ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'ni_20',i,'_WSC.rds'))){
    ### combine urban and nonurban grids
    ni_grid_urban_files <- list.files(path=dir_grid_urban,pattern = paste0("^20",i,"(.*)final.predictions.urban.WSC7.50m.ni.rds$"))
    ...
    ni_grid_nonurban_files <- list.files(path=dir_grid_nonurban,pattern = paste0("^final.predictions.nonurban.WSC7(.*)ni.rds$"))
    ni_grid_nonurban <- readRDS(paste0(dir_grid_nonurban,ni_grid_nonurban_files[1]))
    for (k in 2:length(ni_grid_nonurban_files)) {
      ni_grid_nonurban_tmp <- readRDS(paste0(dir_grid_nonurban,ni_grid_nonurban_files[k]))
      ni_grid_nonurban <- bind_rows(ni_grid_nonurban,ni_grid_nonurban_tmp)
      rm(ni_grid_nonurban_tmp)
      gc()
    }
    ...
    saveRDS(ni_combine,file = paste0(dir_save,'ni_20',i,'_WSC.rds'))
...
############################# 9. no3 ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'no3_20',i,'_WSC.rds'))){
    ### combine urban and nonurban grids
    no3_grid_urban_files <- list.files(path=dir_grid_urban,pattern = paste0("^20",i,"(.*)final.predictions.urban.WSC7.50m.no3.rds$"))
    ...
    no3_grid_nonurban_files <- list.files(path=dir_grid_nonurban,pattern = paste0("^final.predictions.nonurban.WSC7(.*)no3.rds$"))
    no3_grid_nonurban <- readRDS(paste0(dir_grid_nonurban,no3_grid_nonurban_files[1]))
    for (k in 2:length(no3_grid_nonurban_files)) {
      no3_grid_nonurban_tmp <- readRDS(paste0(dir_grid_nonurban,no3_grid_nonurban_files[k]))
      no3_grid_nonurban <- bind_rows(no3_grid_nonurban,no3_grid_nonurban_tmp)
      rm(no3_grid_nonurban_tmp)
      gc()
    }
    ...
    saveRDS(no3_combine,file = paste0(dir_save,'no3_20',i,'_WSC.rds'))
...
############################# 10. oc ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'oc_20',i,'_WSC.rds'))){
    ### combine urban and nonurban grids
    oc_grid_urban_files <- list.files(path=dir_grid_urban,pattern = paste0("^20",i,"(.*)final.predictions.urban.WSC7.50m.oc.rds$"))
    ...
    oc_grid_nonurban_files <- list.files(path=dir_grid_nonurban,pattern = paste0("^final.predictions.nonurban.WSC7(.*)oc.rds$"))
    oc_grid_nonurban <- readRDS(paste0(dir_grid_nonurban,oc_grid_nonurban_files[1]))
    for (k in 2:length(oc_grid_nonurban_files)) {
      oc_grid_nonurban_tmp <- readRDS(paste0(dir_grid_nonurban,oc_grid_nonurban_files[k]))
      oc_grid_nonurban <- bind_rows(oc_grid_nonurban,oc_grid_nonurban_tmp)
      rm(oc_grid_nonurban_tmp)
      gc()
    }
    ...
    saveRDS(oc_combine,file = paste0(dir_save,'oc_20',i,'_WSC.rds'))
...
############################# 11. pb ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'pb_20',i,'_WSC.rds'))){
    ### combine urban and nonurban grids
    pb_grid_urban_files <- list.files(path=dir_grid_urban,pattern = paste0("^20",i,"(.*)final.predictions.urban.WSC7.50m.pb.rds$"))
    ...
    pb_grid_nonurban_files <- list.files(path=dir_grid_nonurban,pattern = paste0("^final.predictions.nonurban.WSC7(.*)pb.rds$"))
    pb_grid_nonurban <- readRDS(paste0(dir_grid_nonurban,pb_grid_nonurban_files[1]))
    for (k in 2:length(pb_grid_nonurban_files)) {
      pb_grid_nonurban_tmp <- readRDS(paste0(dir_grid_nonurban,pb_grid_nonurban_files[k]))
      pb_grid_nonurban <- bind_rows(pb_grid_nonurban,pb_grid_nonurban_tmp)
      rm(pb_grid_nonurban_tmp)
      gc()
    }
    ...
    saveRDS(pb_combine,file = paste0(dir_save,'pb_20',i,'_WSC.rds'))
...
############################# 12. si ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'si_20',i,'_WSC.rds'))){
    ### combine urban and nonurban grids
    si_grid_urban_files <- list.files(path=dir_grid_urban,pattern = paste0("^20",i,"(.*)final.predictions.urban.WSC7.50m.si.rds$"))
    ...
    si_grid_nonurban_files <- list.files(path=dir_grid_nonurban,pattern = paste0("^final.predictions.nonurban.WSC7(.*)si.rds$"))
    si_grid_nonurban <- readRDS(paste0(dir_grid_nonurban,si_grid_nonurban_files[1]))
    for (k in 2:length(si_grid_nonurban_files)) {
      si_grid_nonurban_tmp <- readRDS(paste0(dir_grid_nonurban,si_grid_nonurban_files[k]))
      si_grid_nonurban <- bind_rows(si_grid_nonurban,si_grid_nonurban_tmp)
      rm(si_grid_nonurban_tmp)
      gc()
    }
    ...
    saveRDS(si_combine,file = paste0(dir_save,'si_20',i,'_WSC.rds'))
...
############################# 13. so4 ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'so4_20',i,'_WSC.rds'))){
    ### combine urban and nonurban grids
    so4_grid_urban_files <- list.files(path=dir_grid_urban,pattern = paste0("^20",i,"(.*)final.predictions.urban.WSC7.50m.so4.rds$"))
    ...
    so4_grid_nonurban_files <- list.files(path=dir_grid_nonurban,pattern = paste0("^final.predictions.nonurban.WSC7(.*)so4.rds$"))
    so4_grid_nonurban <- readRDS(paste0(dir_grid_nonurban,so4_grid_nonurban_files[1]))
    for (k in 2:length(so4_grid_nonurban_files)) {
      so4_grid_nonurban_tmp <- readRDS(paste0(dir_grid_nonurban,so4_grid_nonurban_files[k]))
      so4_grid_nonurban <- bind_rows(so4_grid_nonurban,so4_grid_nonurban_tmp)
      rm(so4_grid_nonurban_tmp)
      gc()
    }
    ...
    saveRDS(so4_combine,file = paste0(dir_save,'so4_20',i,'_WSC.rds'))
...
############################# 14. v ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'v_20',i,'_WSC.rds'))){
    ### combine urban and nonurban grids
    v_grid_urban_files <- list.files(path=dir_grid_urban,pattern = paste0("^20",i,"(.*)final.predictions.urban.WSC7.50m.v.rds$"))
    ...
    v_grid_nonurban_files <- list.files(path=dir_grid_nonurban,pattern = paste0("^final.predictions.nonurban.WSC7(.*)v.rds$"))
    v_grid_nonurban <- readRDS(paste0(dir_grid_nonurban,v_grid_nonurban_files[1]))
    for (k in 2:length(v_grid_nonurban_files)) {
      v_grid_nonurban_tmp <- readRDS(paste0(dir_grid_nonurban,v_grid_nonurban_files[k]))
      v_grid_nonurban <- bind_rows(v_grid_nonurban,v_grid_nonurban_tmp)
      rm(v_grid_nonurban_tmp)
      gc()
    }
    ...
    saveRDS(v_combine,file = paste0(dir_save,'v_20',i,'_WSC.rds'))
...
############################# 15. z ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'z_20',i,'_WSC.rds'))){
    ### combine urban and nonurban grids
    z_grid_urban_files <- list.files(path=dir_grid_urban,pattern = paste0("^20",i,"(.*)final.predictions.urban.WSC7.50m.z.rds$"))
    ...
    z_grid_nonurban_files <- list.files(path=dir_grid_nonurban,pattern = paste0("^final.predictions.nonurban.WSC7(.*)z.rds$"))
    z_grid_nonurban <- readRDS(paste0(dir_grid_nonurban,z_grid_nonurban_files[1]))
    for (k in 2:length(z_grid_nonurban_files)) {
      z_grid_nonurban_tmp <- readRDS(paste0(dir_grid_nonurban,z_grid_nonurban_files[k]))
      z_grid_nonurban <- bind_rows(z_grid_nonurban,z_grid_nonurban_tmp)
      rm(z_grid_nonurban_tmp)
      gc()
    }
    ...
    saveRDS(z_combine,file = paste0(dir_save,'z_20',i,'_WSC.rds'))
