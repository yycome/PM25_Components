...
dir_grid_urban <- '/media/qnap3/Exposure modeling/3 National scale/USA/7 PM2.5 speciation v1.2000_19/2 Mid-Atlantic/Urban areas at 50m spatial resolution/'
dir_grid_nonurban <- '/media/qnap3/Exposure modeling/3 National scale/USA/7 PM2.5 speciation v1.2000_19/2 Mid-Atlantic/Non-urban areas at 1km spatial resolution/'
...
############################# 1. br ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'br_20',i,'_MA.rds'))){
    ...
    saveRDS(br_combine,file = paste0(dir_save,'br_20',i,'_MA.rds'))
...
############################# 2. ca ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'ca_20',i,'_MA.rds'))){
    ### combine urban and nonurban grids
    ...
    saveRDS(ca_combine,file = paste0(dir_save,'ca_20',i,'_MA.rds'))
...
############################# 3. cu ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'cu_20',i,'_MA.rds'))){
    ### combine urban and nonurban grids
    ...
    saveRDS(cu_combine,file = paste0(dir_save,'cu_20',i,'_MA.rds'))
...
############################# 4. ec ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'ec_20',i,'_MA.rds'))){
    ### combine urban and nonurban grids
    ...
    saveRDS(ec_combine,file = paste0(dir_save,'ec_20',i,'_MA.rds'))
...
############################# 5. fe ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'fe_20',i,'_MA.rds'))){
    ### combine urban and nonurban grids
    ...
    saveRDS(fe_combine,file = paste0(dir_save,'fe_20',i,'_MA.rds'))
...
for (i in years_char_k) {
  if (!file.exists(paste0(dir_save,'k_20',i,'_MA.rds'))){
    ### combine urban and nonurban grids
    ...
    saveRDS(k_combine,file = paste0(dir_save,'k_20',i,'_MA.rds'))
...
############################# 7. nh4 ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'nh4_20',i,'_MA.rds'))){
    ### combine urban and nonurban grids
    ...
    saveRDS(nh4_combine,file = paste0(dir_save,'nh4_20',i,'_MA.rds'))
...
############################# 8. ni ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'ni_20',i,'_MA.rds'))){
    ### combine urban and nonurban grids
    ...
    saveRDS(ni_combine,file = paste0(dir_save,'ni_20',i,'_MA.rds'))
...
############################# 9. no3 ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'no3_20',i,'_MA.rds'))){
    ### combine urban and nonurban grids
    ...
    saveRDS(no3_combine,file = paste0(dir_save,'no3_20',i,'_MA.rds'))
...
############################# 10. oc ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'oc_20',i,'_MA.rds'))){
    ### combine urban and nonurban grids
    ...
    saveRDS(oc_combine,file = paste0(dir_save,'oc_20',i,'_MA.rds'))
...
############################# 11. pb ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'pb_20',i,'_MA.rds'))){
    ### combine urban and nonurban grids
    ...
    saveRDS(pb_combine,file = paste0(dir_save,'pb_20',i,'_MA.rds'))
...
############################# 12. si ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'si_20',i,'_MA.rds'))){
    ### combine urban and nonurban grids
    ...
    saveRDS(si_combine,file = paste0(dir_save,'si_20',i,'_MA.rds'))
...
############################# 13. so4 ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'so4_20',i,'_MA.rds'))){
    ### combine urban and nonurban grids
    ...
    saveRDS(so4_combine,file = paste0(dir_save,'so4_20',i,'_MA.rds'))
...
############################# 14. v ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'v_20',i,'_MA.rds'))){
    ### combine urban and nonurban grids
    ...
    saveRDS(v_combine,file = paste0(dir_save,'v_20',i,'_MA.rds'))
...
############################# 15. z ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'z_20',i,'_MA.rds'))){
    ### combine urban and nonurban grids
    ...
    saveRDS(z_combine,file = paste0(dir_save,'z_20',i,'_MA.rds'))
