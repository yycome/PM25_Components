...
dir_grid_urban <- '/media/qnap3/Exposure modeling/3 National scale/USA/7 PM2.5 speciation v1.2000_19/3 North East Central/Urban areas at 50m spatial resolution/'
dir_grid_nonurban <- '/media/qnap3/Exposure modeling/3 National scale/USA/7 PM2.5 speciation v1.2000_19/3 North East Central/Non-urban areas at 1km spatial resolution/'
...
############################# 1. br ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'br_20',i,'_NEC.rds'))){
    ### combine urban and nonurban grids
    br_grid_urban_files <- list.files(path=dir_grid_urban,pattern = paste0("^20",i,"(.*)final.predictions.urban.NEC3.50m.br.rds$"))
    ...
    br_grid_nonurban_files <- list.files(path=dir_grid_nonurban,pattern = paste0("^final.predictions.nonurban.NEC3(.*)br.rds$"))
    ...
    saveRDS(br_combine,file = paste0(dir_save,'br_20',i,'_NEC.rds'))
...
############################# 2. ca ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'ca_20',i,'_NEC.rds'))){
    ### combine urban and nonurban grids
    ca_grid_urban_files <- list.files(path=dir_grid_urban,pattern = paste0("^20",i,"(.*)final.predictions.urban.NEC3.50m.ca.rds$"))
    ...
    ca_grid_nonurban_files <- list.files(path=dir_grid_nonurban,pattern = paste0("^final.predictions.nonurban.NEC3(.*)ca.rds$"))
    ...
    saveRDS(ca_combine,file = paste0(dir_save,'ca_20',i,'_NEC.rds'))
...
############################# 3. cu ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'cu_20',i,'_NEC.rds'))){
    ### combine urban and nonurban grids
    cu_grid_urban_files <- list.files(path=dir_grid_urban,pattern = paste0("^20",i,"(.*)final.predictions.urban.NEC3.50m.cu.rds$"))
    ...
    cu_grid_nonurban_files <- list.files(path=dir_grid_nonurban,pattern = paste0("^final.predictions.nonurban.NEC3(.*)cu.rds$"))
    ...
    saveRDS(cu_combine,file = paste0(dir_save,'cu_20',i,'_NEC.rds'))
...
############################# 4. ec ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'ec_20',i,'_NEC.rds'))){
    ### combine urban and nonurban grids
    ec_grid_urban_files <- list.files(path=dir_grid_urban,pattern = paste0("^20",i,"(.*)final.predictions.urban.NEC3.50m.ec.rds$"))
    ...
    ec_grid_nonurban_files <- list.files(path=dir_grid_nonurban,pattern = paste0("^final.predictions.nonurban.NEC3(.*)ec.rds$"))
    ...
    saveRDS(ec_combine,file = paste0(dir_save,'ec_20',i,'_NEC.rds'))
...
############################# 5. fe ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'fe_20',i,'_NEC.rds'))){
    ### combine urban and nonurban grids
    fe_grid_urban_files <- list.files(path=dir_grid_urban,pattern = paste0("^20",i,"(.*)final.predictions.urban.NEC3.50m.fe.rds$"))
    ...
    fe_grid_nonurban_files <- list.files(path=dir_grid_nonurban,pattern = paste0("^final.predictions.nonurban.NEC3(.*)fe.rds$"))
    ...
    saveRDS(fe_combine,file = paste0(dir_save,'fe_20',i,'_NEC.rds'))
...
############################# 6. k ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'k_20',i,'_NEC.rds'))){
    ### combine urban and nonurban grids
    k_grid_urban_files <- list.files(path=dir_grid_urban,pattern = paste0("^20",i,"(.*)final.predictions.urban.NEC3.50m.k.rds$"))
    ...
    k_grid_nonurban_files <- list.files(path=dir_grid_nonurban,pattern = paste0("^final.predictions.nonurban.NEC3(.*)k.rds$"))
    ...
    saveRDS(k_combine,file = paste0(dir_save,'k_20',i,'_NEC.rds'))
...
############################# 7. nh4 ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'nh4_20',i,'_NEC.rds'))){
    ### combine urban and nonurban grids
    nh4_grid_urban_files <- list.files(path=dir_grid_urban,pattern = paste0("^20",i,"(.*)final.predictions.urban.NEC3.50m.nh4.rds$"))
    ...
    nh4_grid_nonurban_files <- list.files(path=dir_grid_nonurban,pattern = paste0("^final.predictions.nonurban.NEC3(.*)nh4.rds$"))
    ...
    saveRDS(nh4_combine,file = paste0(dir_save,'nh4_20',i,'_NEC.rds'))
...
############################# 8. ni ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'ni_20',i,'_NEC.rds'))){
    ### combine urban and nonurban grids
    ni_grid_urban_files <- list.files(path=dir_grid_urban,pattern = paste0("^20",i,"(.*)final.predictions.urban.NEC3.50m.ni.rds$"))
    ...
    ni_grid_nonurban_files <- list.files(path=dir_grid_nonurban,pattern = paste0("^final.predictions.nonurban.NEC3(.*)ni.rds$"))
    ...
    saveRDS(ni_combine,file = paste0(dir_save,'ni_20',i,'_NEC.rds'))
...
############################# 9. no3 ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'no3_20',i,'_NEC.rds'))){
    ### combine urban and nonurban grids
    no3_grid_urban_files <- list.files(path=dir_grid_urban,pattern = paste0("^20",i,"(.*)final.predictions.urban.NEC3.50m.no3.rds$"))
    ...
    no3_grid_nonurban_files <- list.files(path=dir_grid_nonurban,pattern = paste0("^final.predictions.nonurban.NEC3(.*)no3.rds$"))
    ...
    saveRDS(no3_combine,file = paste0(dir_save,'no3_20',i,'_NEC.rds'))
...
############################# 10. oc ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'oc_20',i,'_NEC.rds'))){
    ### combine urban and nonurban grids
    oc_grid_urban_files <- list.files(path=dir_grid_urban,pattern = paste0("^20",i,"(.*)final.predictions.urban.NEC3.50m.oc.rds$"))
    ...
    oc_grid_nonurban_files <- list.files(path=dir_grid_nonurban,pattern = paste0("^final.predictions.nonurban.NEC3(.*)oc.rds$"))
    ...
    saveRDS(oc_combine,file = paste0(dir_save,'oc_20',i,'_NEC.rds'))
...
############################# 11. pb ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'pb_20',i,'_NEC.rds'))){
    ### combine urban and nonurban grids
    pb_grid_urban_files <- list.files(path=dir_grid_urban,pattern = paste0("^20",i,"(.*)final.predictions.urban.NEC3.50m.pb.rds$"))
    ...
    pb_grid_nonurban_files <- list.files(path=dir_grid_nonurban,pattern = paste0("^final.predictions.nonurban.NEC3(.*)pb.rds$"))
    ...
    saveRDS(pb_combine,file = paste0(dir_save,'pb_20',i,'_NEC.rds'))
...
############################# 12. si ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'si_20',i,'_NEC.rds'))){
    ### combine urban and nonurban grids
    si_grid_urban_files <- list.files(path=dir_grid_urban,pattern = paste0("^20",i,"(.*)final.predictions.urban.NEC3.50m.si.rds$"))
    ...
    si_grid_nonurban_files <- list.files(path=dir_grid_nonurban,pattern = paste0("^final.predictions.nonurban.NEC3(.*)si.rds$"))
    ...
    saveRDS(si_combine,file = paste0(dir_save,'si_20',i,'_NEC.rds'))
...
############################# 13. so4 ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'so4_20',i,'_NEC.rds'))){
    ### combine urban and nonurban grids
    so4_grid_urban_files <- list.files(path=dir_grid_urban,pattern = paste0("^20",i,"(.*)final.predictions.urban.NEC3.50m.so4.rds$"))
    ...
    so4_grid_nonurban_files <- list.files(path=dir_grid_nonurban,pattern = paste0("^final.predictions.nonurban.NEC3(.*)so4.rds$"))
    ...
    saveRDS(so4_combine,file = paste0(dir_save,'so4_20',i,'_NEC.rds'))
...
############################# 14. v ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'v_20',i,'_NEC.rds'))){
    ### combine urban and nonurban grids
    v_grid_urban_files <- list.files(path=dir_grid_urban,pattern = paste0("^20",i,"(.*)final.predictions.urban.NEC3.50m.v.rds$"))
    ...
    v_grid_nonurban_files <- list.files(path=dir_grid_nonurban,pattern = paste0("^final.predictions.nonurban.NEC3(.*)v.rds$"))
    ...
    saveRDS(v_combine,file = paste0(dir_save,'v_20',i,'_NEC.rds'))
...
############################# 15. z ##############################
for (i in years_char) {
  if (!file.exists(paste0(dir_save,'z_20',i,'_NEC.rds'))){
    ### combine urban and nonurban grids
    z_grid_urban_files <- list.files(path=dir_grid_urban,pattern = paste0("^20",i,"(.*)final.predictions.urban.NEC3.50m.z.rds$"))
    ...
    z_grid_nonurban_files <- list.files(path=dir_grid_nonurban,pattern = paste0("^final.predictions.nonurban.NEC3(.*)z.rds$"))
    ...
    saveRDS(z_combine,file = paste0(dir_save,'z_20',i,'_NEC.rds'))
